...
class LogicalLoss(nn.Module):
    def __init__(self, hyp, model, distiller='l2', task='detect') -> None:
        ...
        if distiller == 'BCKD':
            self.logical_loss.assigner = TaskAlignedAssigner(topk=10, num_classes=de_parallel(model).nc, alpha=1.5, beta=6.0)
            self.logical_loss.assigner_o2o = TaskAlignedAssigner(topk=1, num_classes=de_parallel(model).nc, alpha=1.5, beta=6.0)
