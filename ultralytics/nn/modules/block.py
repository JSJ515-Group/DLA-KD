...
class EMA(nn.Module):
    def __init__(self, channels, factor=8):
        ...
        self.conv1 = Conv(channels // self.groups, channels // self.groups, k=1, s=1, p=0)
    ...
    def forward(self, x):
        ...
        # self.conv1x1.weight = nn.Parameter(self.conv1x1.weight.to(x_h.dtype))
        # print("x_h dtype:", x_h.dtype)
        # print("x_w dtype:", x_w.dtype)
        # print("conv1x1 weight dtype:", self.conv1.weight.dtype)
        ...
        ema = (group_x * weights.sigmoid()).reshape(b, c, h, w)
        # se = self.se(ema)
        return ema
...
class SimFusion_3in(nn.Module):
    def __init__(self, in_channel_list, out_channels):
        ...
        self.cv2 = Conv(in_channel_list[0], out_channels, act=nn.ReLU()) if in_channel_list[
                                                                                0] != out_channels else nn.Identity()
    ...
    def forward(self, x):
       ...
       # x0 = self.cv1(self.downsample(x[0], output_size))
        #x1 = x[0]
        x1 = self.cv2(x[0])
        x2 = self.cv3(F.interpolate(x[1], size=(H, W), mode='bilinear', align_corners=False))
        x3 = self.cv3(F.interpolate(x[2], size=(H, W), mode='bilinear', align_corners=False))
        return self.cv_fuse(torch.cat((x1, x2, x3), dim=1))
